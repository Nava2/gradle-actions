...
export function setupCaching(
    ...
    const cacheCleaner = supplied?.cacheCleaner ?? new CacheCleaner(gradleProvisioner)
    const cacheAccessor = supplied?.remoteCacheAccessor ?? new RemoteCacheAccessor()
    const cacheKeyGenerator = supplied?.cacheKeyGenerator ?? new CacheKeyGenerator()
    const cacheContentFactory =
        supplied?.cacheContentFactory ??
        new CacheContentFactory(cacheConfig, cacheAccessor, cacheKeyGenerator, cacheCleaner)
